...
class setting extends Parent_Controller {
	...
	public function index(){
		...
		$data['konten'] = 'setting/setting_view';
  	...
  	public function fetch_setting(){  
       $getdata = $this->m_setting->fetch_setting();
	...
	public function hapus_data(){
		$id = $this->uri->segment(3);  
    	$delete = $this->db->where('id',$id)->delete('m_setting');
    ...
    $data_form = $this->m_setting->array_from_post($this->daftar_field);

    $id = isset($data_form['id']) ? $data_form['id'] : NULL; 
 

    $simpan_data = $this->m_setting->simpan_data($data_form,$this->nama_tabel,$this->primary_key,$id);
    ...
    $sql = $this->db->query("select a.*,b.nama_komp_biaya,c.nama_pelayanan,d.nama_satuan,e.nama_setting as parent from m_setting a
              ...
              LEFT JOIN m_setting e on e.id = a.id_parent_setting")->result();
    $arr = array();
    foreach ($sql as $key => $value) {
      $sub_arr = array();
      $sub_arr['nama_pelayanan'] = $value->nama_pelayanan; 
      $sub_arr['nama_komp_biaya'] = $value->nama_komp_biaya; 
      $sub_arr['nama_setting'] = $value->nama_setting; 
